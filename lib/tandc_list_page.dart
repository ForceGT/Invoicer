...
class TandCListPage extends StatefulWidget {
  ...
  TandCListPage({isSelectable,invoice}):_isSelectable=isSelectable,_invoice=invoice;
}
...
class _TandCListPageState extends State<TandCListPage> {
  ...
  Widget getEmptyTermsView(BuildContext context,AsyncSnapshot<List<TandC>> snapshot){
    return Scaffold(
      body: Container(
  ...
  Widget getTermsListView(BuildContext context,AsyncSnapshot<List<TandC>> snapshot){
    return Scaffold(
      appBar: AppBar(
              ...
              child: Card(
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Flexible(
                          flex: 2, child: Icon(Icons.analytics_outlined,size: 34,)),
                      Flexible(
                          flex: 4,
                          child: Text("${snapshot.data[index].terms}",textAlign: TextAlign.center,style: TextStyle(fontWeight: FontWeight.bold),)),
                      Wrap(
                        children: [
                          Column(
                            children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      dialogShowCompleted = await showDialog(
                                          context: context,
                                          builder: (context) {
                                            return buildAlertDialog(snapshot, index);
                                          });
                                      // print(
                              ...
                              Container(
                                  decoration: BoxDecoration(
                                    ...
                                    color: snapshot.data[index].type == "default"?Colors.blue:Colors.white,
                                  ),
                                  padding: EdgeInsets.all(8.0),
                                  child: snapshot.data[index].type == "default"? Text("${snapshot.data[index].type.toUpperCase()}",
                                    style: TextStyle(
                                      color: Colors.white,
                                    ),
                                  ):null
  ...
  Widget getSelectableTermsListView(BuildContext context,AsyncSnapshot<List<TandC>> snapshot){
    return Scaffold(
      appBar: AppBar(
                ...
                child: Card(
                  ...
                  child: Padding(
                      ...
                      child: CheckboxListTile(
                        ...
                        value: true,
                        onChanged: null,
                        title: Text("${snapshot.data[index].terms}",textAlign: TextAlign.center,style: TextStyle(fontWeight: FontWeight.bold),),
                        secondary: Container(
                            width: 75,
                            decoration: BoxDecoration(
                              shape: BoxShape.rectangle,
                            ...
                            child: Text("${snapshot.data[index].type.toUpperCase()}",
                              style: TextStyle(
                                color: Colors.white,
                              ),
                            )
                  ...
                  child: Padding(
                      ...
                      child: CheckboxListTile(
                        ...
                        onChanged: (value){
                          ...
                          setState(() {
                            if (checkedTerms.any((checkedService) => checkedService == true)){
                              anyTermChecked = true;
                            }else{
                              anyTermChecked=false;
                            }
                            checkedTerms.removeRange(index+1,checkedTerms.length);
                          });
                        },
                        title: Text("${snapshot.data[index].terms}",textAlign: TextAlign.center,style: TextStyle(fontWeight: FontWeight.bold),),
                      )
      ...
      floatingActionButton:Card(
        ...
        child: AnimatedCrossFade(
          ...
          crossFadeState: anyTermChecked?CrossFadeState.showSecond:CrossFadeState.showFirst,
          ...
          secondChild: FloatingActionButton(
            ...
            onPressed: ()async{
              checkedTerms.removeRange(snapshot.data.length, checkedTerms.length);
              if(tAndCIds.isNotEmpty)
                tAndCIds.clear();
              ...
              // print("Widget Invoice Product Ids: ${widget._invoice.listofProductIds}");
              // print("Widget Invoice TAndCId: ${widget._invoice.tAndCId}");

              Invoice.insertInvoice(widget._invoice);
              Navigator.push(context, MaterialPageRoute(builder: (context) => getInvoicePdfPreview(widget._invoice)));
            },
          ),
        ),
      ),
