...
class ServiceListPage extends StatefulWidget {
  ...
  ServiceListPage({isSelectable,invoice}):_isSelectable=isSelectable,_invoice=invoice;
}
...
class _ServiceListPageState extends State<ServiceListPage>{
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () async{
                          ...
                          dialogShowCompleted = await showDialog(context: context,builder: (context){
                            return buildDialog(context, snapshot, index);
                          });
                          print("DialogShowCompleted:$dialogShowCompleted");
                          if(dialogShowCompleted){
                            var resultText = result == 1 ? "Deleted Successfully":"Failed to delete,Please try later";
                            Scaffold.of(context).showSnackBar(SnackBar(content: Text("$resultText")));
                          }
  ...
  Widget buildDialog(BuildContext context, AsyncSnapshot<List<Service>> snapshot, int index){
    return AlertDialog(
      title: Text("Delete ${snapshot.data[index].name}?"),
      content: Text("Are you sure you want to delete this item?"),
      actions: [
        MaterialButton(child: Text("OK"),onPressed: () async{
    ...
    return Scaffold(
                ...
                child: CheckboxListTile(
                  ...
                  onChanged: (value){
                    ...
                    setState(() {
                      ...
                      if (checkedServices.any((checkedService) => checkedService == true)){
                        anyItemChecked = true;
                      }else{
                        anyItemChecked=false;
                      }
                      checkedServices.removeRange(index+1,checkedServices.length);
                    });
                  },
                  title: Text(
                    snapshot.data[index].name,
      ...
      floatingActionButton:Card(
        ...
        child: AnimatedCrossFade(
          ...
          crossFadeState: anyItemChecked?CrossFadeState.showSecond:CrossFadeState.showFirst,
          ...
          secondChild: FloatingActionButton(
            ...
            onPressed: (){
              var totalAmount =0;
              checkedServices.removeRange(snapshot.data.length, checkedServices.length);
