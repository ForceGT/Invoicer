                      ...
                      Padding(
                          ...
                          child: Center(
                            child: RaisedButton(
                              ...
                              child: Text("Import Email From PhoneBook".toUpperCase(),
                                style: TextStyle(
                                    fontSize: 14, color: Colors.white),),
                              onPressed: () async {
                                var permissionGranted = await FlutterContactPicker.requestPermission();
                                if(permissionGranted){

                                    final EmailContact contact = await FlutterContactPicker.pickEmailContact(askForPermission: true);
                                    _emailController.text = contact.email.email;
                                }
                                else{
                                  Scaffold.of(bc).showSnackBar(SnackBar(content: Text("Permission Not Granted..Try Manual Filling")));
                                }
                    ...
                    Padding(
                        ...
                        child: Center(
                          child: RaisedButton(
                            ...
                            color: Colors.blueGrey,
                            textColor: Colors.white,
                            child: Text("Import Contact From PhoneBook".toUpperCase(),
                              style: TextStyle(
                                  fontSize: 14, color: Colors.white),),
                            onPressed: () async {
                              var permissionGranted = await FlutterContactPicker.requestPermission();
                              if(permissionGranted){
                                if(Platform.isIOS){
                                  ...
                                  _phoneNoController.text = contact.phoneNumber.number;
                                }
                                if(Platform.isAndroid){
                                  final FullContact contact = await FlutterContactPicker
                                      .pickContact(askForPermission: true);
                                  _nameController.text = contact.name.firstName + " " + contact.name.lastName;
                                  _phoneNoController.text = contact.phones.first.number;
                        ...
                        child: RaisedButton(
                          ...
                          onPressed: () {
                            ...
                            if (widget._isEdit == true) {
                              ...
                              Scaffold.of(bc).showSnackBar(SnackBar(
                                  content: Text(
                                      "Inserted new client successfully!")));
                            }
